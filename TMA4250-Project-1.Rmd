...
estimate_cond_realization <- function(sigma_N, n) {
  ...
  y <- as.vector(mvrnorm(n=1, rep(0, 3), Sigma[1:3, 1:3] + sigma_N*identity(3)))
...
cond_sim <- array(rep(0, prod(array_dims)), array_dims)
for (i in seq_along(sigma_N)) {
  cond_sim[i, ,] <- estimate_cond_realization(sigma_N=sigma_N[i], n=100)
}
```
...
melted_cond_sim <- cond_sim %>%
  melt() %>%
  group_by(Var1, Var2) %>%
  summarize(mean_est=mean(value), CI90_est=1.64*sd(value)/10) %>%
  nest(-Var1)
